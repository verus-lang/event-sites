...
impl RwLockPredicate<HashMapWithView<Args, Output>> for Pred {
    open spec fn inv(self, v: HashMapWithView<Args, Output>) -> bool {
        // EXERCISE: fill me in
        forall |args| v@.dom().contains(args) ==> v@[args] == func(&args)
    }
}
...
impl Memoizer {
        ...
        ensures out == func(args)
    {
        ...
        let (mut map, handle) = self.lock.acquire_write();

        let c = match map.get(args) {
            Some(output) => output.clone(),
            None => {
                let output = expensive_function(args);
                map.insert(args.clone(), output.clone());
                output
            }
        };

        handle.release_write(map);
