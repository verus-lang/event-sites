...
tokenized_state_machine!{FifoQueue<T> {
    fields {
        ...
        // Represents the shared `head` field (shared via an atomic)
        #[sharding(variable)]
        pub head: nat,

        // Represents the shared `tail` field (shared via an atomic)
        #[sharding(variable)]
        pub tail: nat,

        // Represents the local, owned state of the single producer
        #[sharding(variable)]
        pub producer: ProducerState,

        // Represents the local, owned state of the single consumer
        #[sharding(variable)]
